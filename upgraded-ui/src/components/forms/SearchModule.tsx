...
const SearchModule: React.FC<SearchModuleProps> = ({
  ...
  const [localGenomicTerm, setLocalGenomicTerm] = useState(predefinedGenomicTerm || '');
  const [localFilteringTerm, setLocalFilteringTerm] = useState(predefinedFilteringTerm || '');
  ...
  const [activeSection, setActiveSection] = useState<'genomic' | 'filtering'>('genomic');
  ...
  useEffect(() => {
    if (filterQuery) {
      const filtered = allFilteringTerms.filter(term => term.label?.toLowerCase().includes(filterQuery.toLowerCase()));
  ...
  const addGenomicQuery = () => {
    if (genomicQuery) {
      setGenomicParametersToSearch(prev => [...prev, `${selectedGenome}: ${genomicQuery}`]);
  ...
  return (
    <div className="search-module">
      <div className="search-inputs" ref={inputRef}>
        ...
        <div className={`genomic-query ${activeSection === 'genomic' ? 'active' : ''}`}>
        ...
        <div className={`filtering-query ${activeSection === 'filtering' ? 'active' : ''}`}>
          ...
          <div className="filter-input">
            ...
            />
            {filterQuery && <span className="clear-filter" onClick={clearFilterQuery}>×</span>} {/* Clear button */}
            <button>+</button>
          </div>
          ...
          {filterSuggestions.length > 0 && (
            <ul className="suggestions-box">
              {filterSuggestions.map((term, index) => (
                <li key={index} onClick={() => handleAddFilterTerm(term.id, term.label || term.id)}>
                  {term.label || term.id}
                </li>
              ))}
            </ul>
